"""
Tattva.AI — Metadata / EXIF Analyzer (v2 — Upgraded)
...
def analyze_metadata(file_path: str) -> dict:
    ...
    if not has_exif:
        # Check if image exhibits standard social media stripping signatures
        # (Heuristic: WhatsApp/IG often compress to specific resolutions without EXIF)
        try:
            w, h = pil_image.size
            if w <= 1600 and h <= 1600:
                details.append("No EXIF data — consistent with standard social media privacy stripping (Exempted from penalty)")
            else:
                risk_score += 20
                ai_indicators.append("No EXIF metadata found on uncompressed image")
        except Exception:
            risk_score += 20
            ai_indicators.append("No EXIF metadata found on uncompressed image")
    else:
