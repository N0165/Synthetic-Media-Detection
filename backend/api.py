...
# Concurrency limit to prevent GPU out-of-memory
GPU_SEMAPHORE = asyncio.Semaphore(2)

async def run_async(func, *args, **kwargs):
    """Run a CPU-bound function in a separate thread with GPU concurrency limits."""
    if GPU_SEMAPHORE.locked():
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail="AI inference engine is at maximum capacity. Please retry in a moment."
        )
...
# ── Filename-based AI tool signatures removed (Phase 1 upgrade) ────────
...
def _do_preload():
    ...
    try:
        from detectors.audio_detector import preload_models as preload_audio_models
        preload_audio_models()
        print("[API] Audio models successfully preloaded.")
    except Exception as e:
...
async def detect_image_endpoint(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Filename-based override removed per Phase 1 upgrade
