"""
Tattva.AI — Explanation Generator & AI Insights Engine
...
def generate_ai_insights(result: dict, media_type: str = "image") -> dict:
    ...
    return {
        ...
        "summary": summary,
        "vlm_explanation": _generate_vlm_explanation_prompt(result, insights, summary)
    }

def _generate_vlm_explanation_prompt(result: dict, insights: list, summary: str) -> str:
    """
    Phase 2 Update: VLM Integration.
    Prepares a context-rich prompt for a Vision-Language Model (like LLaVA or GPT-4V)
    to generate a natural language explanation of the forensic findings.
    """
    verdict = result.get("verdict", "UNKNOWN")
    return (
        f"The forensic engine has classified this media as {verdict}. "
        f"Summary: {summary} "
        f"Key forensic insights: {[i['description'] for i in insights[:2]]}. "
        "In a full deployment, this context is passed to a Vision-Language Model (VLM) "
        "to generate a detailed, user-friendly forensic report."
    )
...
def explain_image_result(result: dict) -> str:
    ...
    # Add VLM explainer hook
    md += "\n### 🤖 VLM Forensic Explainer (Phase 2)\n"
    md += f"*{_generate_vlm_explanation_prompt(result, [], 'View raw insights above')}*\n"
