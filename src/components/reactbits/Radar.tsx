...
void main() {
  ...
  // Alpha processing - if intensity is above threshold, use it as alpha, else mix bg
  float alpha = clamp(intensity, 0.0, 1.0);
  if (length(uBgColor) > 0.0) {
      col += uBgColor * (1.0 - alpha);
      alpha = 1.0; 
  } else {
...
export default function Radar({
  ...
  useEffect(() => {
    ...
    // Set clear color to transparent void
